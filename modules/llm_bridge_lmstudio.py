...
class LMStudioBridge:
    ...
    def gui_log(self, text):
        """Single source of truth for logging - prevents duplication"""
        print(text)
        if self.gui:
            try:
                # Let the GUI handle timestamping
    ...
    def _health_loop(self):
        while True:
            ...
            try:
                ...
                if self.online:
                    self.update_gui_status("🟢 Server ONLINE", "#44ff44")
                else:
                    self.update_gui_status(f"⚠️ HTTP {response.status_code}", "#ffff00")
            except requests.exceptions.ConnectionError:
                self.online = False
                self.update_gui_status("❌ Server OFFLINE", "#ff4444")
            except Exception:
                self.online = False
                self.update_gui_status("❌ Connection Error", "#ff4444")
    ...
    def _apply_to_avatar(self, text: str):
        ...
        with state_lock:
            if any(w in text_lower for w in ["happy", "love", "yay", "good", "excited"]):
                state["CoreGlow"] = 0.92
                state["SensoryGlow"] = 0.78
